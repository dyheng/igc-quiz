    ...
    @elseif ($state === 'running' && $current)
        <div wire:poll.10s="refresh">
            ...
            <div class="sticky top-14 md:top-16 -mx-4 md:-mx-6 px-4 md:px-6 py-3 bg-white/95 backdrop-blur border-b border-slate-200 z-20">
                <div class="flex items-center justify-between gap-3">
                    <div class="flex items-center gap-2">
                        <span class="badge-brand">{{ $currentIndex + 1 }} / {{ $totalQuestions }}</span>
                    </div>
                    @if ($session->ends_at)
                        <div
                ...
                <div class="mt-2 h-1 rounded-full bg-slate-100 overflow-hidden">
                    <div class="h-full bg-brand-500 transition-all" style="width: {{ ($currentIndex + 1) / max(1, $totalQuestions) * 100 }}%"></div>
                </div>
            </div>

            <div class="mt-6">
                <x-ui.card>
                    <p class="label">Pertanyaan {{ $currentIndex + 1 }}</p>
                    <h2 class="text-lg md:text-xl font-medium text-slate-900 mt-1.5 leading-relaxed">{{ $current->text }}</h2>

                    <div
                        class="mt-5 space-y-2.5"
                        wire:key="opts-{{ $current->id }}"
                        x-data="{ chosen: @js($answers[$current->id] ?? null), saving: false }"
                    >
                        @foreach ($current->options as $i => $opt)
                            <button
                                type="button"
                                x-on:click="if (saving) return; chosen = {{ $opt->id }}; saving = true; $wire.selectOption({{ $current->id }}, {{ $opt->id }}).finally(() => saving = false)"
                                x-bind:disabled="saving && chosen !== {{ $opt->id }}"
                                x-bind:class="chosen === {{ $opt->id }} ? 'border-brand-500 bg-brand-50 ring-2 ring-brand-200' : 'border-slate-200 bg-white hover:border-brand-300 hover:bg-slate-50'"
                                class="w-full text-left flex items-center gap-3 p-3 md:p-4 rounded-xl border transition min-h-14 disabled:opacity-60"
                            >
                                <span
                                    x-bind:class="chosen === {{ $opt->id }} ? 'bg-brand-600 text-white' : 'bg-slate-100 text-slate-700'"
                                ...
                                <template x-if="chosen === {{ $opt->id }}">
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-5 h-5 text-brand-600 shrink-0"><path fill-rule="evenodd" d="M16.704 4.153a.75.75 0 01.143 1.052l-8 10.5a.75.75 0 01-1.127.075l-4.5-4.5a.75.75 0 011.06-1.06l3.894 3.893 7.48-9.817a.75.75 0 011.05-.143z" clip-rule="evenodd"/></svg>
                                </template>
                            </button>
                        @endforeach
                    </div>
            ...
            <div class="sticky bottom-0 -mx-4 md:-mx-6 mt-6 px-4 md:px-6 py-3 bg-white/95 backdrop-blur border-t border-slate-200" style="padding-bottom: calc(env(safe-area-inset-bottom) + 0.75rem);">
                {{--
                    [FORWARD-ONLY MODE]
                    Tombol Prev di-disable agar peserta tidak bisa kembali mengubah jawaban
                    saat admin menampilkan dashboard real-time.

                    Untuk mengaktifkan navigasi mundur kembali:
                      1) Uncomment blok <div class="flex items-center justify-between gap-2"> ... </div> di bawah ini
                      2) Comment / hapus blok @if ... @endif single-button setelahnya
                      3) Uncomment method prev() di app/Livewire/PlayQuiz.php
                --}}

                {{--
                <div class="flex items-center justify-between gap-2">
                    <x-ui.button variant="secondary" wire:click="prev" :disabled="$currentIndex === 0">
                    ...
                    @if ($currentIndex === $totalQuestions - 1)
                        <x-ui.button x-on:click="$dispatch('open-modal', 'confirm-finish')">
                            <span>Selesai</span>
                ...
                @if ($currentIndex === $totalQuestions - 1)
                    <x-ui.button x-on:click="$dispatch('open-modal', 'confirm-finish')" class="w-full btn-lg">
                        <span>Selesai</span>
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-4 h-4"><path fill-rule="evenodd" d="M16.704 4.153a.75.75 0 01.143 1.052l-8 10.5a.75.75 0 01-1.127.075l-4.5-4.5a.75.75 0 011.06-1.06l3.894 3.893 7.48-9.817a.75.75 0 011.05-.143z" clip-rule="evenodd"/></svg>
                    </x-ui.button>
                @else
                    <x-ui.button wire:click="next" class="w-full btn-lg">
                        <span>Selanjutnya</span>
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-4 h-4"><path fill-rule="evenodd" d="M8.22 5.22a.75.75 0 011.06 0l4.25 4.25a.75.75 0 010 1.06l-4.25 4.25a.75.75 0 11-1.06-1.06L11.94 10 8.22 6.28a.75.75 0 010-1.06z" clip-rule="evenodd"/></svg>
                    </x-ui.button>
                @endif
